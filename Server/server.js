...
io.on('connection', function(socket){ //This is the server part of the "what happens when we first connect" function. Everytime a user connects a instance of this is set up for the user privatley
    ...
    socket.on('nodeTemperature', function(data) {

        console.log('Received the temperature ' + data + ' C');

    });

    socket.on('nodePressure', function(data) {

        console.log('Received the pressure ' + data + ' hPa');

    });

    socket.on('nodeHumidity', function(data) {

        console.log('Received the relative humidity ' + data + ' %');

    });

    socket.on('nodeVOC', function(data) {

        console.log('Received the gas resistance ' + data + ' kOhm');
